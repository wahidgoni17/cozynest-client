...
const ManageUsersPage = () => {
  ...
  const [selectedRow, setSelectedRow] = useState<{ id: string; status: string; role: string } | null>(null);
  ...
  const columns: GridColDef[] = [
    ...
    { field: "role", headerName: "Role", flex: 1 },
    {
      ...
      renderCell: ({ row }) => (
        <Box>
          <IconButton onClick={() => handleOpenStatusModal(row)} aria-label="edit">
            <EditIcon />
          </IconButton>
        </Box>
      ),
    },
    {
      ...
      renderCell: ({ row }) => (
        <Box>
          <IconButton onClick={() => handleOpenRoleModal(row)} aria-label="role">
            <EditIcon />
          </IconButton>
        </Box>
      ),
